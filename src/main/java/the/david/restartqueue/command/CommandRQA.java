    ...
    if (args.length == 2) {
        ...
        if (offlinePlayer.hasPlayedBefore()) {
          ...
          for (Object o : json) {
            ...
            if (uuid.toString().equalsIgnoreCase(obj.get("creator").toString())) {
              ...
              player.sendMessage(obj.get("name").toString() + ", XYZ: " + (String)location.get(0) + " " + (String)location.get(1) + " " + (String)location.get(2));
            }
          }
          return true;
        }
      ...
      if (offlinePlayer.hasPlayedBefore()) {
        ...
        for (Object o : json) {
          JSONObject obj = (JSONObject)o;
          if (uuid.toString().equalsIgnoreCase(obj.get("creator").toString()) &&
                  args[2].equalsIgnoreCase(obj.get("name").toString())) {
            List<String> location = new ArrayList<>();
            JSONArray loc = (JSONArray)obj.get("location");
            for (int i = 0; i < 3; i++)
              location.add(loc.get(i).toString());
            World world = Bukkit.getWorld(obj.get("world_name").toString());
            Location loca = new Location(world, Integer.parseInt(location.get(0)) + 0.5D, Integer.parseInt(location.get(1)), Integer.parseInt(location.get(2)) + 0.5D, 0.0F, 90.0F);
